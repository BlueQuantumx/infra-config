...
Defines the OpenTofu-managed personal infrastructure contract — the Azure Linux VM and its networking, Cloudflare DNS records, the nixos-anywhere NixOS provisioning step, remote state layout, and gitignored subscription handling — lifted out of any single host directory into a top-level `tofu/` capability. The flake exposes `tofuVars` and the `tofu-vars` / `tofu-plan` / `tofu-apply` apps as personal-infra-wide helpers rather than an azure-host concern.
