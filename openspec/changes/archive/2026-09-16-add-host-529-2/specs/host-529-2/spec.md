# Delta spec: host-529-2

## Purpose

定义物理桌面主机 `529-2`（x86_64-linux）的最小接入契约：flake 输出可求值、磁盘布局由 disko 声明、SSH 与用户基线复用共享模块、sops 接收者以占位形式预留，per-host 文件以可求值的占位内容交付，具体机器相关值留待填写。
