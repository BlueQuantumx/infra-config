...
sing-box 配置模板中，`route.default_domain_resolver` SHALL 指向一个加密的、经 direct 出站的 DNS 服务器（`dns-direct`，DoH 223.5.5.5）。该服务器自身 SHALL 使用 IP 地址作为 `server`（避免引导递归），且 SHALL 显式设置 `detour: direct-out`。
